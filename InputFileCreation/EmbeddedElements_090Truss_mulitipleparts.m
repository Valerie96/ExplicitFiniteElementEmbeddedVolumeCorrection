...
OriginalINP = 'INPfiles/DyneemaCohesive-Shot1.inp';
PartName = "TargetPlate";
OutsideNSet = "AllOutside"; 
smallest_elt_length = 1E-3;

NewINP = "DyneemaCohesive-Shot1-1395FpT.inp"; 

OriginalINP = 'INPfiles/Dyneema-3PtBend.inp';
PartName = "BeamSlice";
OutsideNSet = "Outside"; 
smallest_elt_length = 1E-3;

NewINP = "Dyneema-3PtBend-6000FpT.inp"; 
...
[Outside,Nodes,Elements,INSTANCE]=CollectInfo(ELSET,INSTANCE,NSET,PART,STEP,OutsideNSet);
...
%Determine Point Cloud Points that are outside the part
    XIN=inpolyhedron(PartSurf,Nodes(:,:),PointCloudX);
    YIN=inpolyhedron(PartSurf,Nodes(:,:),PointCloudY);
...
function PlotMeshShape(Nodes,Outside,PartSurf,PointCloudX,PointCloudY,PLOT)
    if PLOT
        ...
        patch('Vertices',Nodes(:,1:3), 'Faces', PartSurf ,'FaceColor','g','FaceAlpha',0.2);
...
%         ylim([-0.11,0.11]);
        zlim([0 0.005]);
        patch('Vertices',Nodes(:,:), 'Faces', PartSurf ,'FaceColor','k','FaceAlpha',0.1,'LineWidth',.5);
        for i=1:size(Outside,1)
            plot3(Outside(i,2), Outside(i,3), Outside(i,4), 'k.');
        end
    hold off;
